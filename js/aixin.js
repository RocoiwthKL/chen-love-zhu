class CreatePattern {
	...
	giveBirthToAChild() {
		...
		const randomColor = this.options.palette[Math.floor(Math.random() * this.options.palette.length)]
		const randomTranslateValue = () => (Math.random() < 0.5 ? -1 : 1) * (this.options.distanceMin + Math.random() * (
			this.options.distanceMax - this.options.distanceMin))
